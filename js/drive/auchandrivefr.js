			...
			var $nodes = $( newNodes ); // jQuery set
					$nodes.each(function() {
						...
						if( $node.hasClass( "product-item" ) ) {
							...
							$node.find(".product-item__shortcuts").append("<img class='product-item__shortcutsButton  openfood' src='"+localThis.DriveOffLocal.clickImgFile+"'>");
						}
					});
				}
			}); 
...
auchandrivefr.prototype.addingExtension = function(){
	...
	$(".product-item__shortcuts").append("<img class='product-item__shortcutsButton  openfood' src='"+this.DriveOffLocal.clickImgFile+"'>");
		
	//Ajout dans le cas d'une liste
	if ( $( ".operations-area" ).length ) {
		$(".operations-area").append("<img class='openfood' src='"+this.DriveOffLocal.clickImgFile+"'>");

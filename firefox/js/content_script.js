var url = "https://fr.openfoodfacts.org/cgi/search.pl?search_simple=1&action=process&json=1&appversion="+encodeURIComponent(browser.runtime.getManifest().version)+"&appid="+encodeURIComponent(browser.runtime.getManifest().name);
var _data;
...
function addingExtensionToCarrefour(){
	var img = brower.extension.getURL("/img/openFood.png");
		...
		if(data_.status==1){ 
				$("#productList").append("<div indice='0' class='product'>"
					...
					if(data_.product.image_url===undefined){
						$("#pimg_0").css("background-image","url("+browser.extension.getURL('/img/unknown.jpg')+")");
					}else{
		...
		if(data_.products.length > 0){ 
			for(i=0;i<data_.products.length;i++){
				...
				if(data_.products[i].image_url===undefined){
					$("#pimg_"+i).css("background-image","url("+browser.extension.getURL('/img/unknown.jpg')+")");
				}else{
...
function fillDetail(indice_){
	...
	if(nutriScore===undefined){
		var nutriURL = browser.extension.getURL("/img/unknown.jpg");
	}else{
		var nutriURL  = browser.extension.getURL("/img/nutriscore-"+nutriScore+".svg");
	}
	$(".detail").find("#nutriScore").attr("src",nutriURL);
	
	if(novaScore===undefined){
		var novaURL = browser.extension.getURL("/img/unknown.jpg");
	}else{
		var novaURL  = browser.extension.getURL("/img/nova-group-"+novaScore+".svg");
	}
